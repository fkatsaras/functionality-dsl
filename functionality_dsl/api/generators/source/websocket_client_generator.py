...
def generate_websocket_source_client(source, model, templates_dir, out_dir):
    """
    ...
    if hasattr(source, "operations") and source.operations:
        operations_block = source.operations
        if hasattr(operations_block, "ws_ops"):
            operations = [op for op in operations_block.ws_ops]
