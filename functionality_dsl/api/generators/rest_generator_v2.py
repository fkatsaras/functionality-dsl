...
from ..extractors import get_request_schema, get_response_schema, find_target_for_entity
from ..flow_analyzer_v2 import analyze_endpoint_flow, print_flow_analysis, EndpointFlowType
...
def generate_rest_endpoint(endpoint, model, all_endpoints, all_source_names, templates_dir, output_dir, server_config):
    ...
    2. Classify flow type (COMPUTE_ONLY, READ, WRITE, READ_WRITE)
    ...
    # =========================================================================
    # STEP 5: Build Computation Chain
    # =========================================================================
    compiled_chain = []

    if response_entity:
        # Build transformation chain for response
        compiled_chain = build_entity_chain(response_entity, model, all_source_names, context="ctx")
    elif request_entity and flow.flow_type in {EndpointFlowType.WRITE, EndpointFlowType.COMPUTE_ONLY}:
        # For write-only/compute-only, build chain for request entity
        compiled_chain = build_entity_chain(request_entity, model, all_source_names, context="ctx")
            ...
            for src in get_children_of_type("SourceREST", model):
                if src.name == target_obj["name"]:
                    ...
                    if target_response_schema and target_response_schema["type"] == "entity":
                        ...
                        # Build transformation chain if response needs transformation
                        if response_source_entity.name != response_entity.name:
    ...
    template_context = {
        # Flow information
        "flow": {
            "type": flow.flow_type.value,  # "compute", "read", "write", "read_write"
